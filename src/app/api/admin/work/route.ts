...
import { validateWorkManifest } from '@/lib/content/work';
...
export async function GET(): Promise<Response> {
  await requireAdmin();
  const res = await readManifest<WorkManifest>(FILE);
  if (!res) {
    // Not seeded yet — serve the bundled seed; generation "0" means "create if absent".
    return NextResponse.json({ manifest: SEED_WORK, generation: '0' });
  }
...
export async function PUT(req: Request): Promise<Response> {
  ...
  const check = validateWorkManifest(body.manifest);
  if (!check.ok) {
  ...
  try {
    ...
    if (err instanceof ConflictError) {
      return NextResponse.json(
        { ok: false, error: 'conflict', message: 'work changed elsewhere — reload and retry' },
        { status: 409 },
      );
    }
    console.error('[admin/work] write failed', err);
    return NextResponse.json({ ok: false, error: 'write_failed' }, { status: 500 });
  }

  await publishManifest('work');

  // Re-read so the editor can adopt the fresh generation and publish again without a 409.
  const after = await readManifest<WorkManifest>(FILE);
