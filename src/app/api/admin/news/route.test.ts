...
describe('GET /api/admin/news', () => {
  ...
  it('falls back to the seed with generation "0" when absent', async () => {
    ...
    expect(body.manifest).toEqual(SEED_NEWS);
  });
});

describe('PUT /api/admin/news', () => {
  it('writes with the loaded generation, publishes, and returns the new generation', async () => {
    ...
    expect(writeManifest).toHaveBeenCalledWith('news.json', manifest, { ifGenerationMatch: '3' });
    expect(publishManifest).toHaveBeenCalledWith('news');
  ...
  it('rejects an invalid manifest (unknown status) with 400', async () => {
    const bad = { version: 1, posts: [{ id: 'a', title: 'A', date: '2026-04-06', body: 'hi', status: 'archived', order: 0 }] };
